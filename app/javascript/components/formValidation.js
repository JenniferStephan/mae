...
const toggleActiveClass = (idName) => {
  document.querySelectorAll(`#${idName} label`).forEach((label) => {
    label.addEventListener('click', (event) => {
      removeActiveClass(idName);
      event.currentTarget.classList.add('active');
    });
  });
}

const removeActiveClass = (idName) => {
  document.querySelectorAll(`#${idName} label`).forEach((label) => {
    label.classList.remove('active');
  });
}

const myClasses = document.querySelectorAll('.simulation-choix').forEach((item) => {
